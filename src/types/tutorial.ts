/**
 * 新手引导类型定义（issue #717，移植自 old-python 分支）
 *
 * 引导通过「编排」已有组件和 Store 来引导用户操作：
 * 教程是"导演"，已有组件是"演员"——通过调用 Store 的公开方法驱动界面，
 * 配合 spotlight 高亮和 tooltip 提示引导用户。
 */
...
export interface TutorialStep {
  ...
  /** 点击"下一步"时执行的动作（触发后隐藏教程，等待操作完成） */
  nextAction?: TutorialAction
  /** 与 nextAction 配合：隐藏教程后等待某 store 字段变 false 才继续 */
...
/** 教程模式 */
export type TutorialMode = 'idle' | 'first-launch'
