    ...
    <Transition name="tutorial-fade">
      ...
      >
        ...
        >
          ...
          <!-- 标题 -->
          <h3 class="tutorial-title">{{ t(currentStep.title) }}</h3>

          <!-- 内容（支持 \n 换行） -->
          <p class="tutorial-content">{{ t(currentStep.content) }}</p>
          ...
          <div class="tutorial-actions">
            ...
            <button
              ...
              @click="handleNext"
            >
              {{ tutorialStore.isLastStep ? t('tutorial.done') : t('tutorial.next') }}
            </button>
          </div>
        </div>
      </div>
...
.tutorial-progress-fill {
  height: 100%;
  background: linear-gradient(90deg, #79d9ff, #a78bfa);
  border-radius: 999px;
...
.tutorial-btn--next {
  background: linear-gradient(135deg, #79d9ff, #a78bfa);
  color: #1a1a2e;
}
